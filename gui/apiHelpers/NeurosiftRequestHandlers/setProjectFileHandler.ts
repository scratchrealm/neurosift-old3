...
import { SetProjectFileRequest, SetProjectFileResponse } from "../../src/types/NeurosiftRequest";
import { NSProjectFile } from "../../src/types/neurosift-types";
import getProject from '../getProject';
import { getMongoClient } from "../getMongoClient";
import getWorkspace from '../getWorkspace';
import { userCanSetProjectFile } from '../permissions';

const setProjectFileHandler = async (request: SetProjectFileRequest, o: {verifiedUserId?: string}): Promise<SetProjectFileResponse> => {
    ...
    if (!userCanSetProjectFile(workspace, verifiedUserId)) {
    ...
    if (!projectFile) {
        const newProjectFile: NSProjectFile = {
            projectId,
            workspaceId,
            fileName: request.fileName,
            contentSha1,
